...
def get_tcga_multi_labeled_dataloaders(gex_features_df, drug, batch_size):
    ...
    tissue_counts = {}
    for tissue_id in sample_tissue:
        tissue_counts[tissue_id] = tissue_counts.get(tissue_id, 0) + 1

    return tcga_dataloader, tissue_counts
...
def get_multi_labeled_dataloader(gex_features_df, drug, seed, batch_size, ccle_measurement,
    ...
    tcga_labeled_dataloader, tissue_counts = get_tcga_multi_labeled_dataloaders(
        gex_features_df=gex_features_df, drug=drug, batch_size=batch_size
    )
    ...
    for train_labeled_ccle_dataloader, test_labeled_ccle_dataloader in ccle_labeled_dataloader:
        yield train_labeled_ccle_dataloader, test_labeled_ccle_dataloader, tcga_labeled_dataloader, tissue_counts
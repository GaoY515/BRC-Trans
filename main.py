...
from models import pretraining, finetuning
from models import classifier
from copy import deepcopy
from lib.utility import set_seed_all
import time
import numpy as np
import warnings
...
def main(args, drug, params_dict):
    ...
    for train_labeled_ccle, test_labeled_ccle, labeled_tcga, tissue_counts in labeled_dataloader:
        ...
        if not weights_printed:
            ...
            total_weight = sum(1 / np.sqrt(count) for count in tissue_counts.values())
            tissue_weights = {
                tissue_id: (1 / np.sqrt(count)) / total_weight
                for tissue_id, count in tissue_counts.items()
            }

            print('================Tissue Weights================')
            for tissue_id,_ in sorted(tissue_counts.items(), key=lambda x: x[1]):
                weight = tissue_weights[tissue_id]
                print(f"Tissue ID {tissue_id}: Weight {weight:.4f}")
        ...
        with warnings.catch_warnings():
            ...
            network, final_similarity_matrix = finetuning.training(encoder=ft_encoder,
    ...
    if 'network' in locals() and hasattr(network, 'encoder'):
        try:
            similarity_matrix = network.encoder.contrastive_loss_fn.cancer_similarity().detach().cpu().numpy()
            
            task_save_folder = os.path.join('results', 'similarity_analysis')
            make_dir(task_save_folder)
            np.save(os.path.join(task_save_folder, f'cancer_similarity_matrix_{param_str}.npy'), similarity_matrix)
            ...
            sns.heatmap(
                ...
                vmin=0, 
                vmax=1,
                xticklabels=[f"Type {i}" for i in range(similarity_matrix.shape[0])],
                yticklabels=[f"Type {i}" for i in range(similarity_matrix.shape[0])]
            )
            plt.title('Learned Cancer Type Similarity Matrix', fontsize=16)
            plt.xlabel('Cancer Type', fontsize=14)
            plt.ylabel('Cancer Type', fontsize=14)
            plt.tight_layout()
            plt.savefig(os.path.join(task_save_folder, f'cancer_similarity_heatmap_{param_str}.png'), dpi=300)
            plt.close()
            
            print(f"Similarity matrix saved to {task_save_folder}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alph', dest='alph', nargs='?', type=float, default=0.2)
    parser.add_argument('--beta', dest='beta', nargs='?', type=float, default=0.3)
    parser.add_argument('--lamda', dest='lamda', nargs='?', type=float, default=0.3)
    ...
    for param in params_list:      
        main(args=args, params_dict=param,
             drug=["5-Fluorouracil", "Cyclophosphamide", "Docetaxel", 
